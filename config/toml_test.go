...
func TestTomlNew(t *testing.T) {
	cfg, err := NewConfigWithFormatType(ConfFormatTypeToml, "testdata/test.toml")
	assert(t, err == nil)
	...
	_, err = NewConfigWithFormatType(ConfFormatTypeToml, "testdata/test1.toml")
	assert(t, err != nil)
}

func TestTomlGetSetting(t *testing.T) {
	cfg, err := NewConfigWithFormatType(ConfFormatTypeToml, "testdata/test.toml")
	assert(t, err == nil)
...
func TestTomlGetSettingMust(t *testing.T) {
	cfg, err := NewConfigWithFormatType(ConfFormatTypeToml, "testdata/test.toml")
	assert(t, err == nil)
	for _, tc := range testcases {
		sec, err := cfg.GetSection(tc.sec)
...
func TestTomlGetAllSections(t *testing.T) {
	cfg, err := NewConfigWithFormatType(ConfFormatTypeToml, "testdata/test.toml")
	assert(t, err == nil)
...
func TestTomlLoadAndLastMod(t *testing.T) {
	cfg, err := NewConfigWithFormatType(ConfFormatTypeToml, "testdata/test.toml")
	assert(t, err == nil)
	err = cfg.Load()
	assert(t, err == nil)
